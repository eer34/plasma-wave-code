    program IWNS
		...
		do k=1,40
			...
			do region_i=1,15
				...
				down_edge=-30*k_para_rho_i_para_input
				up_edge=4.0_wp 
				...
				if (my_id==0) then
					do n=1,z_solve_number
						...
						write(fid,'(*(G30.7,:,",",X))') k_para_rho_i_para_input,k_per_rho_i_para_input,c_div_v_para_input,n,real(ans_z_solve(n)),aimag(ans_z_solve(n)),ans_mul_solve(n),ans_z_error(n),abs(ans_f_solve(n)),eigen,polar(1),polar(2),polar(3)
