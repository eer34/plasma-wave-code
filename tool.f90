...
!-----------------------------------------------------------------------------!
!     set_parameter_two_ion_species: given k to calculate omega for two ion species plasma
!-----------------------------------------------------------------------------!    
		subroutine set_parameter_two_ion_species(c_div_v_para_input,omega_pe_div_omega_ce_input,k_para_rho_i_para_input,k_per_rho_i_para_input,k_per_rho_i_per_input,k_para_rho_e_para_input,k_per_rho_e_para_input,k_per_rho_e_per_input,k_para_rho_d_para_input,k_per_rho_d_para_input,k_per_rho_d_per_input)
			implicit none
			real(wp),intent(in)::c_div_v_para_input,omega_pe_div_omega_ce_input,k_para_rho_i_para_input,k_para_rho_e_para_input,k_per_rho_i_para_input,k_per_rho_i_per_input,k_per_rho_e_para_input,k_per_rho_e_per_input,k_para_rho_d_para_input,k_per_rho_d_para_input,k_per_rho_d_per_input
			...
			k_per_rho_e_per=k_per_rho_e_per_input
			k_per_rho_e_para=k_per_rho_e_para_input
			k_para_rho_d_para=k_para_rho_d_para_input
			k_per_rho_d_para=k_per_rho_d_para_input
			k_per_rho_d_per=k_per_rho_d_per_input
		end subroutine set_parameter_two_ion_species
...
!-----------------------------------------------------------------------------!
!     dispersion_function_two_ion_species_matrix:the dispersion matrix for omega with known k in two ion species plasma.
!-----------------------------------------------------------------------------!
    subroutine dispersion_function_two_ion_species_matrix(x,D)
		implicit none
		complex(wp),intent(in)::x
		complex(wp),intent(out)::D(:,:)
		complex(wp)::series_sum_1,series_sum_2,x_pdf,y_pdf
		complex(wp)::xe_pdf_1,xe_pdf_2,ye_pdf_1,ye_pdf_2
		complex(wp)::xd_pdf_1,xd_pdf_2,yd_pdf_1,yd_pdf_2
		integer::n,k
		real(wp)::gamma_n,gamma_n1,gamma_n2
		complex(wp)::x_e,x_d
		...
		case(1)
			...
			do n=1,20
				call bessel_gn(k_per_rho_i_per**2/2,n, gamma_n)
				call bessel_gn(k_per_rho_e_per**2/2,n, gamma_n1)
				call bessel_gn(k_per_rho_d_per**2/2,n, gamma_n2)
				if(abs(k_para_rho_i_para)<1e-6) then
				  series_sum_1=series_sum_1+4/k_per_rho_i_per**2*omega_pi_div_omega_ci*gamma_n*(n**2/(cmplx(n**2,0)-x**2))
				  series_sum_1=series_sum_1+4/k_per_rho_e_per**2*omega_pe_div_omega_ce*gamma_n1*(n**2/(cmplx(n**2,0)-x_e**2))
				  series_sum_1=series_sum_1+4/k_per_rho_d_per**2*omega_pd_div_omega_cd*gamma_n2*(n**2/(cmplx(n**2,0)-x_d**2))
				else
				  x_pdf=(x-cmplx(n,0))/k_para_rho_i_para
				  call pdf(x_pdf,y_pdf)
				  ...
				  x_pdf=(x_d-cmplx(n,0))/k_para_rho_d_para
				  call pdf(x_pdf,y_pdf)
				  
				  series_sum_1=series_sum_1+2*omega_pd_div_omega_cd/x_d*gamma_n2*(n**2/k_per_rho_d_per**2/k_para_rho_d_para*y_pdf+n**2/x_d*(1/k_per_rho_d_para**2-1/k_per_rho_d_per**2)*(1+x_pdf*y_pdf))
		...
		case(2)
			...
			do n=1,20
				...
				if(abs(k_para_rho_i_para)<1e-6) then
					...
					call bessel_gn(k_per_rho_d_per**2/2,n, gamma_n)
					call bessel_gn(k_per_rho_d_per**2/2,n+1,gamma_n1)
					series_sum_1=series_sum_1+(0,1)*omega_pd_div_omega_cd/x_d*(2*n**2)/(cmplx(n**2,0)-x_d**2)*(gamma_n1-(1-2*n/k_per_rho_d_per**2)*gamma_n)
				else
					call bessel_gn(k_per_rho_i_per**2/2,n, gamma_n)
					call bessel_gn(k_per_rho_i_per**2/2,n+1,gamma_n1)
					x_pdf=(x-cmplx(n,0))/k_para_rho_i_para
					...
					series_sum_1=series_sum_1+cmplx(0,omega_pd_div_omega_cd,wp)/x_d*(k_per_rho_d_per**2*(gamma_n1-gamma_n)+2*n*gamma_n)*(n/k_per_rho_d_per**2/k_para_rho_d_para*y_pdf+n/x_d*(1/k_per_rho_d_para**2-1/k_per_rho_d_per**2)*(1+x_pdf*y_pdf))
			...
			if(abs(k_para_rho_i_para)<1e-6) then
			  	D(1,3)=(0,0)
			   	...
			   	do n=1,20
					...
					x_pdf=(x_d-cmplx(n,0))/k_para_rho_d_para
					call pdf(x_pdf,y_pdf)
					series_sum_1=series_sum_1+2*omega_pd_div_omega_cd/x_d*n*gamma_n*k_per_rho_d_para/k_para_rho_d_para*(1/k_per_rho_d_para**2+n/x_d*(1/k_per_rho_d_per**2-1/k_per_rho_d_para**2))*(1+x_pdf*y_pdf)
		...
		case(5)
			...
			do n=0,20
				...
				if(abs(k_para_rho_i_para)<1e-6) then
					...
					call bessel_gn(k_per_rho_d_per**2/2,n, gamma_n)
					call bessel_gn(k_per_rho_d_per**2/2,n+1,gamma_n1)
					series_sum_1=series_sum_1+omega_pd_div_omega_cd/x_d*((k_per_rho_d_per**2-2*n+2*n**2/k_per_rho_d_per**2)*gamma_n-k_per_rho_d_per**2*gamma_n1)/(cmplx(n,0)-x_d)
					if(n>0) then
						series_sum_1=series_sum_1+omega_pd_div_omega_cd/x_d*((k_per_rho_d_per**2-2*n+2*n**2/k_per_rho_d_per**2)*gamma_n-k_per_rho_d_per**2*gamma_n1)/(cmplx(-n,0)-x_d)
					end if
				else
					...
					series_sum_1=series_sum_1+omega_pd_div_omega_cd/x_d*(gamma_n*(k_per_rho_d_per**4-2*n*k_per_rho_d_per**2+2*n**2)-k_per_rho_d_per**4*gamma_n1)*(1/k_per_rho_d_per**2/k_para_rho_d_para*y_pdf+1/x_d*(1/k_per_rho_d_para**2-1/k_per_rho_d_per**2)*(1+x_pdf*y_pdf))
					if(n>0) then
						x_pdf=(x_d+cmplx(n,0))/k_para_rho_d_para
						call pdf(x_pdf,y_pdf)
						series_sum_1=series_sum_1+omega_pd_div_omega_cd/x_d*(gamma_n*(k_per_rho_d_per**4-2*n*k_per_rho_d_per**2+2*n**2)-k_per_rho_d_per**4*gamma_n1)*(1/k_per_rho_d_per**2/k_para_rho_d_para*y_pdf+1/x_d*(1/k_per_rho_d_para**2-1/k_per_rho_d_per**2)*(1+x_pdf*y_pdf))
			...
			if (abs(k_para_rho_i_para)<1e-6) then
			  D(2,3)=(0,0)
			  	...
			  	do n=0,20
					...
					series_sum_1=series_sum_1-cmplx(0,omega_pd_div_omega_cd,wp)/x_d*k_per_rho_d_para/k_para_rho_d_para*(k_per_rho_d_per**2*(gamma_n1-gamma_n)+2*n*gamma_n)*(1/k_per_rho_d_para**2+n/x_d*(1/k_per_rho_d_per**2-1/k_per_rho_d_para**2))*(1+x_pdf*y_pdf)
					if(n>0) then
						x_pdf=(x_d+cmplx(n,0))/k_para_rho_d_para
						call pdf(x_pdf,y_pdf)
						series_sum_1=series_sum_1-cmplx(0,omega_pd_div_omega_cd,wp)/x_d*k_per_rho_d_para/k_para_rho_d_para*(k_per_rho_d_per**2*(gamma_n1-gamma_n)+2*n*gamma_n)*(1/k_per_rho_d_para**2-n/x_d*(1/k_per_rho_d_per**2-1/k_per_rho_d_para**2))*(1+x_pdf*y_pdf)
		...
		case(9)
			...
			do n=0,20
				...
				if (abs(k_para_rho_i_para)<1e-6) then
					...
					call bessel_gn(k_per_rho_d_per**2/2,n,gamma_n)
					series_sum_1=series_sum_1+omega_pd_div_omega_cd/x_d*gamma_n/(cmplx(n,0)-x_d)
					...
					x_pdf=(x_d-cmplx(n,0))/k_para_rho_d_para
					call pdf(x_pdf,y_pdf)
					series_sum_1=series_sum_1+2*omega_pd_div_omega_cd*k_per_rho_d_para**2/x_d/k_para_rho_d_para*x_pdf*gamma_n*(1/k_per_rho_d_para**2+n/x_d*(1/k_per_rho_d_per**2-1/k_per_rho_d_para**2))*(1+x_pdf*y_pdf)
		...
		end subroutine dispersion_function_two_ion_species_matrix    


!-----------------------------------------------------------------------------!
!     dispersion_function_two_ion_species:dispersion relation with omega as the variable for two ion species plasma
!-----------------------------------------------------------------------------! 
		complex(wp) function dispersion_function_two_ion_species(x)
		implicit none
		complex(wp),intent(in)::x
		complex(wp)::D(3,3)
		integer::n,k
		...
		dispersion_function_two_ion_species=det(D,3)
	  
		end function dispersion_function_two_ion_species
